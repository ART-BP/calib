...
inline std::string normalizeCameraModel(const std::string& model) {
...
inline bool isFisheyeCameraModel(const std::string& model) {
  const std::string normalized = normalizeCameraModel(model);
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr& colored_cloud) 
{
  ...
  const bool use_fisheye = isFisheyeCameraModel(camera_model);
  cv::Mat samplingImage;
  if (use_fisheye) {
    samplingImage = image;
  } else {
    // For pinhole input, undistort first and then project with zero distortion.
    cv::undistort(image, samplingImage, cameraMatrix, distCoeffs);
  }
  ...
  for (const auto& point : *cloud) 
  {
    ...
    if (u >= 0 && u < samplingImage.cols && v >= 0 && v < samplingImage.rows) 
    {
      // Get the color from the selected sampling image.
      cv::Vec3b color = samplingImage.at<cv::Vec3b>(v, u);
     ...
     const pcl::PointCloud<pcl::PointXYZRGB>::Ptr& colored_cloud, const cv::Mat& img_input)
{
  ...
  if (outFile.is_open()) 
  {
    const bool use_fisheye = isFisheyeCameraModel(params.camera_model);
    double d2 = params.p1;
    double d3 = params.p2;
    if (use_fisheye) {
      d2 = params.k3;
      d3 = params.k4;
      // Backward compatibility: if users still put fisheye k3/k4 into p1/p2.
      if (std::abs(d2) < 1e-12 && std::abs(d3) < 1e-12 &&
    ...
    outFile << "cam_model: " << (use_fisheye ? "Fisheye" : "Pinhole") << "\n";
