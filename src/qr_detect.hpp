...
class QRDetect 
{
  private:
    ...
    void drawAxisGeneric(const cv::Vec3d& rvec, const cv::Vec3d& tvec, double axis_length) 
    {
      if (!is_fisheye_) {
        cv::aruco::drawAxis(imageCopy_, cameraMatrix_, distCoeffs_, rvec, tvec, axis_length);
        return;
      }

      std::vector<cv::Point3f> axis_points = {
          cv::Point3f(0.0f, 0.0f, 0.0f),
          cv::Point3f(static_cast<float>(axis_length), 0.0f, 0.0f),
          cv::Point3f(0.0f, static_cast<float>(axis_length), 0.0f),
          cv::Point3f(0.0f, 0.0f, static_cast<float>(axis_length))};
      std::vector<cv::Point2f> image_points;
      cv::fisheye::projectPoints(axis_points, image_points, rvec, tvec, cameraMatrix_, distCoeffs_);
      if (image_points.size() != 4) return;

      cv::line(imageCopy_, image_points[0], image_points[1], cv::Scalar(0, 0, 255), 2);  // X-red
      cv::line(imageCopy_, image_points[0], image_points[2], cv::Scalar(0, 255, 0), 2);  // Y-green
      cv::line(imageCopy_, image_points[0], image_points[3], cv::Scalar(255, 0, 0), 2);  // Z-blue
    }
                           ...
                           cv::Vec3d& rvec, cv::Vec3d& tvec) 
    {
      ...
      std::vector<cv::Point2f> image_points;
      cv::aruco::getBoardObjectAndImagePoints(board, corners, ids, object_points, image_points);
      if (object_points.size() < 4 || image_points.size() != object_points.size()) {
        return false;
      }

      std::vector<cv::Point2f> undistorted_points;
      cv::fisheye::undistortPoints(image_points, undistorted_points, cameraMatrix_, distCoeffs_);

      const cv::Mat identity_k = cv::Mat::eye(3, 3, CV_64F);
      const cv::Mat zero_d = cv::Mat::zeros(4, 1, CV_64F);
      bool ok = cv::solvePnP(object_points, undistorted_points, identity_k, zero_d,
                             rvec, tvec, false, cv::SOLVEPNP_ITERATIVE);
      if (!ok) {
        ok = cv::solvePnP(object_points, undistorted_points, identity_k, zero_d,
  ...
  public:
    ...
    bool is_fisheye() const { return is_fisheye_; }
    ...
    QRDetect(ros::NodeHandle &nh, Params& params) 
    {
      ...
      camera_model_ = normalizeCameraModel(params.camera_model);
      is_fisheye_ = isFisheyeCameraModel(camera_model_);
      ...
      if (is_fisheye_) {
        double k3 = params.k3;
        double k4 = params.k4;
        // Backward compatibility for old configs that store k3/k4 in p1/p2.
        if (std::abs(k3) < 1e-12 && std::abs(k4) < 1e-12 &&
            (std::abs(params.p1) > 1e-12 || std::abs(params.p2) > 1e-12)) {
          k3 = params.p1;
          k4 = params.p2;
          ROS_WARN("[QRDetect] fisheye mode: fallback to p1/p2 as k3/k4.");
        }
    ...
    {      
      ...
      if (ids.size() >= min_detected_markers_ && ids.size() <= TARGET_NUM_CIRCLES) 
      {
        ...
        if (!estimateBoardPose(corners, ids, board, rvec, tvec)) {
          ROS_WARN("[Mono] Unable to estimate board pose.");
          return;
        }
